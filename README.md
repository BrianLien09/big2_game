...
- [🔒 Firebase Spark 房間生命週期與安全規則](#-firebase-spark-房間生命週期與安全規則-security-rules)
- [📝 開發規範與工作流程](#-開發規範與工作流程)
...
### 4. AI 玩家 (BOT) 與 7 款專屬水豚頭像
- **水豚 AI 對手**：房主可隨時在房間中新增 AI 人機（Bot）進行練習，AI 會自動進行合法出牌與策略分析。優化人機決策邏輯，已修正人機思考卡死問題。
- **7 款客製化水豚頭像**：包含呆萌水豚、天才水豚、大老二水豚、墨鏡水豚、溫泉水豚、橘子水豚、紳士水豚等 7 款專屬繪製的漫畫風格水豚頭像，在房間列表與遊玩時即時顯示。支援 GitHub Pages 靜態資源路徑處理與頭像備用字元顯示。

### 5. 行動端極致響應式適配 (Mobile Layout Optimization)
- **動態手牌重疊演算法**：透過 `ResizeObserver` 即時監聽容器寬度，大數量手牌時自動調整卡片重疊間距與大小，保證手牌 100% 收納不跑版。
- **排版高度與按鈕佈局優化**：微調手牌與操作區高度，優化行動端按鈕佈局，並新增獨立的 Pass 按鈕，大幅提升手機玩家操作流暢度並避免誤觸。
- **安全區域適配**：手牌與操作區適配 iOS/Android 的底部 Home 虛擬條，確保按鈕不會被系統導覽列裁切。
- **旁人資訊徽章化**：在手機端將其餘玩家的卡牌渲染為輕量徽章（例如：`🂠 13張`），騰出中央出牌區的呼吸感。
- **結算畫面適配**：修正單局遊戲結束排行榜在手機端的排版跑版問題，使結算資訊在各螢幕尺寸下均能整齊呈現。

### 6. 多人房間管理與自動清理 (Firebase Spark 免費版極致優化)
- **Transaction 原子性退出與加入**：重寫房內人員更新，利用 `runTransaction()` 確保多人同時退出或頁面重整時，玩家資料 Order 與 Map 保持完美同步，解決重複加入與幽靈玩家問題。
- **玩家正常退出邏輯**：新增完善的玩家正常退出與離房機制，妥善處理對局結束或中斷時的清理工作。
- **無感節流清理**：在使用者成功進入大廳、建立房間前、加入房間前，呼叫防抖清理功能 `cleanupExpiredRoomsIfNeeded()`，利用 `sessionStorage` 限制每 30 分鐘冷卻時間，每次最多僅批次刪除 20 間房間，保障日常讀寫在免費額度內。
- **時間欄位合併寫入**：在所有對局寫入中（出牌、Pass、準備、startGame、房主移轉等），時間欄位 `updatedAt` 與 `expiresAt` 直接合併至原本 `updateDoc`，不增添任何多餘的資料庫寫入開銷。
- **零殘留管理**：房主斷線或離房時，房主身份會自動移交給下一個在線玩家；當房內最後一人離開時，Transaction 會自動在資料庫刪除該房間文檔。
...
### 8. 積分賽制與終局結算
- **自訂目標積分**：房主可在建立房間與等待大廳中自由設定 10、15、20 目標積分，當有玩家累計總分達到目標時即結束房間遊戲。
- **生存淘汰機制**：對局結束條件調整至剩下一人，並新增積分結算名次表，依據最終累計積分排出名次。
- **終局恭喜畫面**：特別適配行動端的「終局恭喜」一屏自適應視窗，依據獲勝人數動態顯示單人或多人恭喜資訊，並提供一鍵重新開局功能。

### 9. 點擊喚醒音效系統 (Audio Setup)
- **瀏覽器自動播放政策繞過**：實作點擊互動喚醒 AudioContext 機制，完美修復因瀏覽器政策阻擋而無法播放音效的問題。
- **關鍵音效更新**：修復出牌、Pass 等即時動作音效，並將單局結束與整局結束的音效替換為更高音質的 MP3 格式。

### 10. 斷線快速重連與防呆浮水印
- **對局快速重連**：支援玩家在遊玩中途因網路波動斷線或不慎重新整理頁面時，能快速重連回原房間繼續進行對局。
- **房間號浮水印**：在出牌區中央背景新增淡雅的房號浮水印，便於辨識及截圖交流。
...
- **五張牌型**：
  ...
  - **鐵支 (Four of a Kind)**：4 張相同點數搭配 1 張單牌（可壓過順子、葫蘆等一般 5 張牌型）。
  - **同花順 (Straight Flush)**：5 張點數連續且花色相同的牌（可壓過鐵支及其他一般 5 張牌型）。
  - *註：已修復鐵支與同花順怪物牌型的壓牌邏輯，確保比牌時大牌型可合法壓制小牌型，符合大老二正統規則。*
...
## 🔒 Firebase Spark 房間生命週期與安全規則 (Security Rules)

為適應 Firebase Spark 免費版限制（無 Cloud Functions 與自動 TTL 排程服務），專案特化了一套無伺服器開銷的清理機制：

1. **時間戳記**：房間建立時，寫入 `createdAt`、`updatedAt`，以及比當前時間多 6 小時的 `expiresAt`（過期時間戳記）。
2. **自動延長**：任何玩家在房內做出的變更（準備、開始、出牌、Pass、再玩一局），均會隨該次對局寫入更新 `updatedAt` 與 `expiresAt`，確保活躍對局不被過期清除。
3. **Firestore 安全規則**：配置於 `firestore.rules` 中，限定只有已登入的使用者能建立房間、讀寫房間；而刪除權限被限制在「房間已過期 (`resource.data.expiresAt <= request.time`)」或「房內最後一人退出且該人為當前使用者」的情況，確保資料庫安全。
