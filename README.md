...
**CardDuel** 是一個基於 **Next.js (App Router)** 與 **Firebase (Realtime Database & Auth)** 打造的線上即時多人紙牌對戰平台，目前支援四款經典桌牌遊戲：
...
| 🃏 **鬥地主 (Dou Dizhu)** | 3 人 | 叫地主、搶底牌，以地主對抗兩位農民的牌型壓制對局 |
...
- **鬥地主**：三人 54 張牌制，支援叫 1–3 分、底牌揭示、地主／農民陣營、炸彈與火箭倍率、春天結算，以及專屬人機。

### 2. 規則教學與互動練習
- **大老二壓牌練習**：透過固定題目練習單張、對子與怪物牌型的壓制規則，即時提示錯誤原因。
- **十三支防倒水練習**：辨識前、中、後墩的強度關係，使用實際遊戲判定回饋倒水原因。
- **鬥地主完整牌型表**：提供 14 種系統支援牌型的組成、限制與例子，並附壓牌判斷題。

### 3. 漫畫風格 UI (Neo-brutalism Style)
- 高對比度白底黑字、加粗黑色外框、帶偏移的硬陰影與大字重排版。
- 專屬「水豚載入動畫 (Capybara Loader)」在大廳及對局切換時呈現溫暖動態感。
- 漫畫風 Toast 提醒系統，帶微旋轉線稿質感與物理沉降手感。

### 4. 即時對局同步 (Real-time Sync)
...
### 鬥地主 (Dou Dizhu)
- **發牌與叫分**：使用 54 張牌，三人各 17 張、保留 3 張底牌；依序叫 0–3 分，最高分成為地主並取得底牌。全員不叫時重新發牌。
- **點數大小**：`3` < `4` < … < `A` < `2` < 小王 < 大王。
- **一般壓制規則**：需以相同牌型、相同張數（連牌還需相同連長）且主牌更大的組合壓制；順子、連對與飛機主體不可含 `2` 或王。
- **支援牌型**：單張、對子、三條、三帶一、三帶二、順子、連對、飛機、飛機帶單、飛機帶對、四帶二、四帶兩對、炸彈與火箭。
- **特殊牌型**：炸彈可壓任何一般牌型；火箭（小王＋大王）為全場最大。每次炸彈或火箭會使倍率 ×2。
- **結算**：底注 × 叫分 × 倍率計算單家輸贏；地主勝且兩位農民未出牌，或農民勝且地主只出過一次，即觸發春天再 ×2。
- **互動規則頁**：在遊戲大廳點擊「了解規則」可開啟各模式的互動教學與練習。
...
│   │   └── landlord-tutorial/       # 鬥地主牌型規則與互動練習頁面
