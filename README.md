...
- **水豚 AI 對手**：房主可隨時在房間中新增 AI 人機（Bot）進行練習，AI 會自動進行合法出牌與策略分析。
- **7 款客製化水豚頭像**：包含呆萌水豚、天才水豚、大老二水豚、墨鏡水豚、溫泉水豚、橘子水豚、紳士水豚等 7 款專屬繪製的漫畫風格水豚頭像，在房間列表與遊玩時即時顯示。
...
- **時間欄位合併寫入**：在所有對局寫入中（出牌、Pass、準備、startGame、房主移轉等），時間欄位 `updatedAt` 與 `expiresAt` 直接合併至原本的 `updateDoc`，不增添任何多餘的資料庫寫入開銷。
