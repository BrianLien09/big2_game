...
exports.selectBridgeCardPlay = exports.selectBridgeBid = exports.selectBotAction = exports.findStrongCombos = void 0;
...
const findStrongCombos = (botCards) => {
    const combos = [];
    const groups = {};
    botCards.forEach(c => {
        if (!groups[c.rank])
...
exports.findStrongCombos = findStrongCombos;
// 檢查候選牌是否破壞了任何強組合
const breaksStrongCombo = (cardsToPlay, strongCombos) => {
...
// 產生單張候選
const generateSingles = (botCards) => {
    return botCards.map(c => [c]);
};
// 產生對子候選
const generatePairs = (botCards) => {
    const pairs = [];
    const groups = {};
    botCards.forEach(c => {
        if (!groups[c.rank])
...
// 產生葫蘆候選
const generateFullHouses = (botCards) => {
    const fullHouses = [];
    const groups = {};
    botCards.forEach(c => {
        if (!groups[c.rank])
...
// 產生鐵支候選
const generateFourOfAKinds = (botCards) => {
    const fourOfAKinds = [];
    const groups = {};
    botCards.forEach(c => {
        if (!groups[c.rank])
...
// 產生順子與同花順候選
const generateStraightsAndStraightFlushes = (botCards) => {
    const straights = [];
...
// 選擇最佳候選牌的評分與過濾
const selectBestPlay = (candidates, prevHand, strongCombos, firstPlayRequiredCardId) => {
    ...
    const validCandidates = candidates.filter(combo => {
        ...
        // 檢查牌型是否合法
        const hand = (0, big2Logic_1.evaluateHand)(combo);
        if (!hand)
            return false;
        // 檢查是否能壓過上一手
        return (0, big2Logic_1.canPlay)(combo, prevHand);
    });
    ...
    validCandidates.sort((a, b) => {
        ...
        const handA = (0, big2Logic_1.evaluateHand)(a);
        const handB = (0, big2Logic_1.evaluateHand)(b);
        // 如果是怪物牌型 (鐵支或同花順)
        ...
        if (handA.type === 'four_of_a_kind' && handB.type === 'four_of_a_kind') {
            return (0, big2Logic_1.getFourOfAKindRank)(handA) - (0, big2Logic_1.getFourOfAKindRank)(handB);
        }
        return (0, big2Logic_1.compareSingleCard)(handA.keyCard, handB.keyCard);
    });
    return validCandidates[0];
};
// Bot 決策主入口
const selectBotAction = (botCards, prevHand, firstPlayRequiredCardId) => {
    const strongCombos = (0, exports.findStrongCombos)(botCards);
...
exports.selectBotAction = selectBotAction;
const BRIDGE_RANK_WEIGHT_BOT = {
...
const isBidHigher = (nLevel, nSuit, lLevel, lSuit) => {
    if (nLevel !== lLevel)
        return nLevel > lLevel;
 ...
 */
const selectBridgeBid = (botCards, lastContract) => {
    ...
    for (let lvl = lastContract.level; lvl <= 7; lvl++) {
        if (isBidHigher(lvl, botFavoredSuit, lastContract.level, lastContract.suit)) {
            ...
            return { type: 'contract', level: lvl, suit: botFavoredSuit };
        }
        // 試試同線位更高的花色 (或者 NT)
        if (isBidHigher(lvl, 'NT', lastContract.level, lastContract.suit)) {
            ...
            return { type: 'contract', level: lvl, suit: 'NT' };
        }
    }
    return { type: "PASS" };
};
exports.selectBridgeBid = selectBridgeBid;
// ====================================================
 ...
 */
const selectBridgeCardPlay = (botCards, leadCard, trumpSuit) => {
...
exports.selectBridgeCardPlay = selectBridgeCardPlay;
