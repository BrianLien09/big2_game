...
### 6.19 大老二重置狀態時準備與手牌未清空，且結算切換存在競態與動畫殘留閃爍
*   **Bug 症狀**：大老二單局結束時，在播放收牌飛牌動畫的 2.6 秒等待期內，因為畫面仍渲染在打牌桌面且不能操作，顯得像卡死或有 Bug，且在 2.0 秒時桌面會突然跳出「【某某某】收牌」字樣（贏家頭像/提示直接出來）。而當點擊「再玩一局」後，因為大老二只將房間狀態設回 `"waiting"` 而未清空真人的 `isReady` 狀態及卡牌，導致大廳一閃而過秒開局，且在開局瞬間因為資料不同步，畫面會短暫渲染出不完整的 bug 打牌桌面，隨後才恢復正常。
*   **發現技巧**：在大老二結束後點擊「再玩一局」，仔細觀察單局結束期間的卡死感以及重開局時畫面的瞬間跳動閃爍。
*   **原因分析**：
    1. 結算等待動畫有 2.6 秒延遲，該延遲內無任何遮罩覆蓋，玩家仍能看見已無法操作的殘留牌桌。
    2. 大老二舊的重置邏輯直接使用 `update` 修改 `status: "waiting"`，未像十三支和傷心小棧那樣清空玩家手牌與將真人重設為未準備狀態。
    3. 發牌開局時 `status` 優先變回 `"playing"`，在卡牌加載前產生了極短暫的時序差，渲染出不完整的對局狀態。
*   **修復步驟**：
